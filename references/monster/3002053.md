# 王下一桶（投影）（Lordly Trashcan (Projection)）

> 数据来源：[nanoka](https://hsr.nanoka.cc/monster/3002053)（测试站 4.4.51 数据）

## 基础信息

| 项目 | 值 |
|------|-----|
| 怪物 ID | 3002053 |
| 中文名 | 王下一桶（投影） |
| 英文名 | Lordly Trashcan (Projection) |
| 等级类型 | 杂兵 |
| 弱点属性 | — |

## 基础属性（随敌人等级缩放，此为基准值）

| 项目 | 值 |
|------|-----|
| 生命值 | 9300 |
| 攻击力 | 18 |
| 防御力 | 210 |
| 速度 | 144 |
| 韧性 | 300（韧性条 1） |
| 效果抵抗 | 0.1 |
| 相位数 | 1 |

---

## 技能

### 桶的锤炼

> 对我方单体造成物理属性伤害。

### 桶的磨砺

> 提高自身的攻击力。该效果可叠加，且不可被解除。

### 桶的信条

> 当自身被弱点击破时将会击破所有除【王下一桶（投影）】外的敌方目标的弱点，同时自身离开战斗。当受到我方攻击时，将会被添加此次攻击对应的属性弱点。


---

## 描述

> A peculiar entity born within the realm of fiction, its story documented in the nameless chronicles: A lone and valiant trashcan bearing the mission of the Garbage King to seek formidable warriors to eliminate all waste that threatens the world.
