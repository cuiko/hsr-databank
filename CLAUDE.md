...
**新增过角色 / 光锥时，提交前必须跑一遍 `python3 scripts/check_consistency.py`**：核对档案与 `mapping-char2lc.md` 是否同步。只加档案、忘了补映射表不会报任何错，但会让查找链从"精确命中"退化成"匹配到近似条目"——例如查「知更鸟•晴歌」却拿到「知更鸟」的数据，答案看起来完整可信，实际是另一个角色。别靠人工记得这一步。

### 推送前版本号更新

每次执行 `git push` 前，需要获取当前正式服版本号，更新 README.md 顶部的版本徽标。版本号格式：正式服用 `vx.y`（如 `v4.3`），测试服用 `vx.y.z`（如 `v4.3.51`）。README 徽标为正式服版本，故用 `vx.y`。
...
- 按「推送前版本号更新」更新 README.md 顶部版本徽标为 `vx.y` 格式。
